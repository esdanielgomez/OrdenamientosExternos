 ...
 * @author iJonna
 */
public class MezclaBalanceadaa {
    ...
    MezclaBalanceadaa(File archivo, int campo) {
...
escribir(archivo_leer);
        ...
        //distribbuciÃ³n inicial en tramos desde archivo origen
        try{
            t = distribuir(archivo_leer);
            for(i = 0; i < N; i++)
                c[i] = i;
            //bucle hasta numero ded tramos == 1: archivo ordenado
            do{
                k1 = (t < N2) ? t : N2;
                ...
                t = 0;
                for(i = j; i < N;i++)
                    flujos[c[i]] = new CsvWriter(new FileWriter(nomAuxs[c[i]], true), ',');
    ...
    private int distribuir(String archivo_leer) throws IOException{
        ...
        for (j = 0; j < N2; j++) {
            flujoSalida[j] = new CsvWriter(new FileWriter(archivos[j], true), ',');          
        }
        ...
        while (archivoPrincipal.readRecord()) {
            ...
            while(CompararCampos(anterior, clave) == true) {
                ...
                flujoSalida[j].write(archivoPrincipal.get(0));
                flujoSalida[j].write(archivoPrincipal.get(1));
                flujoSalida[j].write(archivoPrincipal.get(2));
                flujoSalida[j].write(archivoPrincipal.get(3));
    ...
    private void escribir(String archivo_leer) {
        try { 
            ...
            System.out.println("arch leeer: "+archivo_leer);
            leer_datos.close();
            
        }catch (IOException e) {
            System.out.println("Error entrada/salia durante el proceso "+ "de ordenaciÃ³n " );
            e.printStackTrace();
        }  
    } 
